...
@st.cache_data
def get_data():
    all_sales = pd.read_excel(r"C:\Users\mikej\Desktop\unfi\weekly_sales_and_cbs.xlsx")
    # conn = st.connection('dot', type ="sql")
    # all_sales = conn.query("SELECT * FROM level_2 WHERE date > '2022-12-31'")
    return all_sales

all_sales = get_data()
all_sales = all_sales[all_sales.chain != 'CONFIDENTIAL']

st.markdown("#")

unique_chains = list(all_sales.chain.unique())
unique_chains_sorted = sorted(filter(None, unique_chains))
...
def filter_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with modification_container:
        ...
        for column in to_filter_columns:
            ...
            if is_categorical_dtype(df[column]) or column == 'year' or column == 'week' or column == 'chain' or column == 'store' or df[column].nunique() < 10:
...
chain_sales = round(df.groupby('chain',as_index=False)['sales'].sum()).sort_values(by='sales',ascending=False)

fig_chain_sales = px.pie(
    chain_sales, 
    values='sales', 
    names='chain',
...
fig_chain_sales.update_traces(textposition='inside', textinfo='percent+label', texttemplate='%{label}<br>%{percent:.0%}',hovertemplate = '%{label}<br>%{value:.2s}')

## monthly bar chart
# df.index = pd.to_datetime(df['date'])
weekly_sales = df.groupby(['year','week'])[['sales']].sum()

fig_wk_bar = px.bar(weekly_sales.reset_index(),
        template='plotly_dark',
        x= 'week',
        y='sales',
        # labels = {'date':' ','usd':''},
...
fig_wk_bar.update_traces(marker_color='#E09641',texttemplate='%{text:$,.2s}',hovertemplate = '$%{y:.2s}'+'<br>%{x:%Y-%m}<br>')
# fig_wk_bar.update_xaxes(showgrid=False,gridcolor='gray',tickvals = weekly_sales.index,ticktext=weekly_sales.index.strftime('%b-%y'),tickfont=dict(color='#5A5856', size=13),title_font=dict(color='#5A5856',size=15))
fig_wk_bar.update_yaxes(showgrid=False,tick0=0,dtick=250000,showticklabels=False, tickcolor='darkgrey', gridcolor='darkgrey')   
...
with col1:
    ...
    st.markdown("###")
    # df = filter_dataframe(all_sales)
    st.markdown("<h2>UNFI Sales</h2>",unsafe_allow_html=True)
    ...
    # st.markdown("#")
with col2:
    st.plotly_chart(fig_chain_sales,config=config,use_container_width=True)

tab1, tab2, tab3 = st.tabs(["Monthly", "Weekly", "YoY"])
with tab1:
    st.plotly_chart(fig_wk_bar, config=config,legend=None, use_container_width=True)
with tab2:
    st.plotly_chart(fig_wk_bar, config=config,use_container_width=True)
with tab3:
    st.plotly_chart(fig_chain_sales, config=config,use_container_width=True)
