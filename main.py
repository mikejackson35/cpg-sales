...
@st.cache_data
def get_direct():
    direct_sales = pd.read_csv(r'data/direct_sales.csv')
...
@st.cache_data
def get_true():
    true_sales = pd.read_csv(r"data/true_sales.csv")
...
df = true_sales.groupby([true_sales.date,'cust_segment']).amount.sum().reset_index().set_index('date')
